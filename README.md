...
| **Asset Type** | Corporate Action Event |
| **Control Record** | Corporate Action Event Procedure |
...
| POST | `/v1/corporate-action-event-procedure/initiate` | Initiate |
| GET | `/v1/corporate-action-event-procedure` | Retrieve (list) |
| GET | `/v1/corporate-action-event-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/corporate-action-event-procedure/{crId}/update` | Update |
| PUT | `/v1/corporate-action-event-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/corporate-action-event-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-corporate-action:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-corporate-action ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-corporate-action` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
